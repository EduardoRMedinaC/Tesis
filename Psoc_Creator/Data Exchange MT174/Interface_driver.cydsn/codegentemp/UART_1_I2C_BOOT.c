...
static uint8 UART_1_slReadBuf[UART_1_I2C_BTLDR_SIZEOF_READ_BUFFER];

/* Reads from this buffer */
static uint8 UART_1_slWriteBuf[UART_1_I2C_BTLDR_SIZEOF_WRITE_BUFFER];

/* Flag to release buffer to be read */
static uint32 UART_1_applyBuffer;
...
static void UART_1_I2CResposeInsert(void);


/*******************************************************************************
* Function Name: UART_1_I2CCyBtldrCommStart
********************************************************************************
*
* Summary:
...
{
    UART_1_I2CSlaveInitWriteBuf(UART_1_slWriteBuf, UART_1_I2C_BTLDR_SIZEOF_WRITE_BUFFER);
    UART_1_I2CSlaveInitReadBuf (UART_1_slReadBuf, 0u);

    UART_1_SetCustomInterruptHandler(&UART_1_I2CResposeInsert);
    UART_1_applyBuffer = 0u;
...
/*******************************************************************************
* Function Name: UART_1_I2CCyBtldrCommStop
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: UART_1_I2CCyBtldrCommReset
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: UART_1_I2CCyBtldrCommRead
********************************************************************************
*
* Summary:
...
*
* Global variables:
*  UART_1_slWriteBuf - used to store received command.
...
cystatus UART_1_I2CCyBtldrCommRead(uint8 pData[], uint16 size, uint16 * count, uint8 timeOut)
{
    ...
    if ((NULL != pData) && (size > 0u))
    {
        ...
        while (0u != timeoutMs)
        {
            ...
            if (0u != (UART_1_I2C_SSTAT_WR_CMPLT & UART_1_slStatus))
            {
                /* Copy number of written bytes */
                *count = (uint16) UART_1_slWrBufIndex;
                ...
                (void) memcpy((void *) pData, (const void *) UART_1_slWriteBuf,
                                              UART_1_BYTES_TO_COPY(*count, size));
...
/*******************************************************************************
* Function Name: UART_1_I2CCyBtldrCommWrite
********************************************************************************
*
* Summary:
...
*
* Global variables:
*  UART_1_slReadBuf - used to store response.
*
*******************************************************************************/
cystatus UART_1_I2CCyBtldrCommWrite(const uint8 pData[], uint16 size, uint16 * count, uint8 timeOut)
{
    ...
    if ((NULL != pData) && (size > 0u))
    {
        ...
        *count = size;
        (void) memcpy((void *) UART_1_slReadBuf, (const void *) pData, (uint32) size);

        /* Read buffer is ready to be released to host */
        UART_1_applyBuffer = (uint32) size;
...
/*******************************************************************************
* Function Name: UART_1_I2CResposeInsert
********************************************************************************
*
* Summary:
...
*
* Global variables:
*  UART_1_slRdBufSize - used to store slave read buffer size.
*  UART_1_slRdBufIndex - used to store the current index within the
*  slave read buffer.
*  UART_1_applyBuffer - the flag to release the buffer with
*  a response to read.
*
*******************************************************************************/
static void UART_1_I2CResposeInsert(void)
{
    if (UART_1_CHECK_INTR_SLAVE_MASKED(UART_1_INTR_SLAVE_I2C_ADDR_MATCH))
    {
        if (UART_1_CHECK_I2C_STATUS(UART_1_I2C_STATUS_S_READ))
        {
            ...
            if (0u != UART_1_applyBuffer)
            {
                UART_1_slRdBufSize  = UART_1_applyBuffer;
                UART_1_slRdBufIndex = 0u;
                UART_1_applyBuffer  = 0u;
            }
        }
        else
        {
            ...
            if (UART_1_slRdBufIndex != UART_1_slRdBufSize)
            {
                UART_1_slRdBufIndex = UART_1_slRdBufSize;
            }
        }
    }
}
