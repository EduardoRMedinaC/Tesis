...
#include "UART_1_EZI2C_PVT.h"
    ...
    /*******************************************************************************
    * Function Name: UART_1_EZI2C_STRETCH_ISR
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    CY_ISR_PROTO(UART_1_EZI2C_STRETCH_ISR)
    {
        ...
        static volatile uint8 * UART_1_dataBuffer; /* Pointer to data buffer              */
        static uint16 UART_1_bufSizeBuf;           /* Size of buffer in bytes             */
        static uint16 UART_1_protectBuf;           /* Start index of write protected area */

        static uint8 activeAddress;
        uint32 ackResponse;

        ackResponse = UART_1_EZI2C_ACK_RECEIVED_ADDRESS;
    ...
    #else
        CY_UART_1_CUSTOM_INTR_HANDLER();
    #endif /* !defined (CY_REMOVE_UART_1_CUSTOM_INTR_HANDLER) */

        /* Make local copy of global variable */
        locIndex = UART_1_EZI2C_GET_INDEX(activeAddress);

        /* Get interrupt sources */
        locIntrSlave = UART_1_GetSlaveInterruptSource();
        locIntrCause = UART_1_GetInterruptCause();
        ...
        */
        if(0u != (locIntrSlave & (UART_1_INTR_SLAVE_I2C_ARB_LOST |
                                  UART_1_INTR_SLAVE_I2C_BUS_ERROR)))
        {
            UART_1_CTRL_REG &= (uint32) ~UART_1_CTRL_ENABLED; /* Disable SCB block */

        #if(UART_1_CY_SCBIP_V0)
            if(0u != ((uint8) UART_1_EZI2C_STATUS_BUSY & UART_1_curStatus))
        #endif /* (UART_1_CY_SCBIP_V0) */
            {
                UART_1_curStatus &= (uint8) ~UART_1_EZI2C_STATUS_BUSY;
                UART_1_curStatus |= (uint8)  UART_1_EZI2C_STATUS_ERR;

                /* INTR_TX_EMPTY is enabled in the address phase to receive data */
                if(0u == (UART_1_GetTxInterruptMode() & UART_1_INTR_TX_EMPTY))
                {
                    /* Write complete */
                    if(UART_1_indexBuf1 != UART_1_offsetBuf1)
                    {
                        UART_1_curStatus |= (uint8) UART_1_INTR_SLAVE_I2C_WRITE_STOP;
                    }
                }
                else
                {
                    /* Read complete */
                    UART_1_curStatus |= (uint8) UART_1_INTR_SLAVE_I2C_NACK;
                }
            }
            ...
            UART_1_CTRL_REG |= (uint32) UART_1_CTRL_ENABLED;  /* Enable SCB block */
        }
        else
        {
            /* INTR_I2C_EC_WAKE_UP */
            /* Wakes up device from deep sleep */
            if(0u != (locIntrCause & UART_1_INTR_CAUSE_I2C_EC))
            {
            ...
            if(0u != (locIntrCause & (UART_1_INTR_CAUSE_RX | UART_1_INTR_CAUSE_SLAVE)))
            {
                /* INTR_RX.NOT_EMPTY */
                ...
                if (0u != (UART_1_GetRxInterruptSourceMasked() & UART_1_INTR_RX_NOT_EMPTY))
                {
                    ...
                    */
                    if (0u == (locIntrCause & UART_1_INTR_CAUSE_SLAVE))
                #endif /* (UART_1_SECONDARY_ADDRESS_ENABLE_CONST) */
                    {
                        locByte = UART_1_RX_FIFO_RD_REG;

                        switch(UART_1_fsmState)
                        {
                        case UART_1_EZI2C_FSM_BYTE_WRITE:
                            if(0u != locBufSize)
                            {
                                /* Store data byte and ACK */
                                UART_1_I2C_SLAVE_GENERATE_ACK;

                                UART_1_dataBuffer[locIndex] = (uint8) locByte;
                                locIndex++;
                                locBufSize--;
                            }
                            else
                    ...
                    #if(UART_1_SUB_ADDRESS_SIZE16_CONST)
                        case UART_1_EZI2C_FSM_OFFSET_HI8:
                            ...
                            UART_1_fsmState = UART_1_EZI2C_FSM_OFFSET_LO8;

                            break;
                    #endif /* (UART_1_SUB_ADDRESS_SIZE16_CONST) */

                        case UART_1_EZI2C_FSM_OFFSET_LO8:

                            if (UART_1_SUB_ADDRESS_SIZE16)
                            {
                            ...
                            if(locByte < (uint32) UART_1_bufSizeBuf)
                            {
                                UART_1_I2C_SLAVE_GENERATE_ACK;
                                ...
                                locBufSize = (uint16) ((locByte < UART_1_protectBuf) ?
                                                       (UART_1_protectBuf - locByte) : (0u));

                            #if(UART_1_CY_SCBIP_V0)

                                if(locBufSize < UART_1_EZI2C_FIFO_SIZE)
                                {
                                    /* Set FSM state to receive byte by byte */
                                    UART_1_fsmState = UART_1_EZI2C_FSM_BYTE_WRITE;
                                }
                                /* Receive RX FIFO chunks */
                                else if(locBufSize == UART_1_EZI2C_FIFO_SIZE)
                                {
                                    UART_1_ENABLE_SLAVE_AUTO_DATA; /* NACK when RX FIFO is full */
                                    UART_1_SetRxInterruptMode(UART_1_NO_INTR_SOURCES);
                                }
                                else
                                {
                                    UART_1_ENABLE_SLAVE_AUTO_DATA_ACK; /* Stretch when RX FIFO is full */
                                ...
                                UART_1_EZI2C_SET_OFFSET(activeAddress, locIndex);
                            }
                            else
                            {
                ...
                else if (0u != (UART_1_I2C_CTRL_REG & UART_1_I2C_CTRL_S_READY_DATA_ACK))
                {
                    ...
                    */
                    if(0u != (locIntrCause & UART_1_INTR_CAUSE_SLAVE))
                    {
                        ...
                        fifoIndex = UART_1_GET_RX_FIFO_ENTRIES;
                        ...
                        if(UART_1_SECONDARY_ADDRESS_ENABLE)
                        {
                            ...
                            if(UART_1_CHECK_INTR_SLAVE_MASKED(UART_1_INTR_SLAVE_I2C_ADDR_MATCH))
                            {
                                ...
                                */
                                fifoIndex -= ((0u != UART_1_GET_RX_FIFO_SR_VALID) ? (0u) : (1u));
                            }
                        }
                        

                        UART_1_DISABLE_SLAVE_AUTO_DATA;
                        endTransfer = UART_1_EZI2C_CONTINUE_TRANSFER;
                    }
                    else
                    /* INTR_RX_FULL */
                    /* Continue transfer or disable INTR_RX_FULL to catch completion event. */
                    {
                        ...
                        locBufSize -= (uint16) UART_1_EZI2C_FIFO_SIZE;

                        if(locBufSize <= UART_1_EZI2C_FIFO_SIZE)
                        {
                            /* Send NACK when RX FIFO overflow */
                            fifoIndex   = locBufSize;
                            endTransfer = UART_1_EZI2C_COMPLETE_TRANSFER;
                        }
                        else
                        {
                            /* Continue  transaction */
                            fifoIndex   = UART_1_EZI2C_FIFO_SIZE;
                            endTransfer = UART_1_EZI2C_CONTINUE_TRANSFER;
                        }
                    }
                    ...
                    for(; (0u != fifoIndex); fifoIndex--)
                    {
                        ...
                        UART_1_dataBuffer[locIndex] = (uint8) UART_1_RX_FIFO_RD_REG;
                        locIndex++;
                    }

                    /* Complete transfer sending NACK when RX FIFO overflow */
                    if(UART_1_EZI2C_COMPLETE_TRANSFER == endTransfer)
                    {
            ...
            if(0u != (locIntrCause & UART_1_INTR_CAUSE_SLAVE))
            {
            ...
            #if(!UART_1_SECONDARY_ADDRESS_ENABLE_CONST)
                if(0u != (locIntrSlave & UART_1_INTR_SLAVE_I2C_STOP))
            #else
                /* Prevent triggering when matched address was NACKed */
                if((0u != (locIntrSlave & UART_1_INTR_SLAVE_I2C_STOP)) &&
                   (0u != ((uint8) UART_1_EZI2C_STATUS_BUSY & UART_1_curStatus)))
            #endif
                {
                    /* Disable TX and RX interrupt sources */
                    UART_1_SetRxInterruptMode(UART_1_NO_INTR_SOURCES);
                    ...
                    locStatus = UART_1_INTR_SLAVE_I2C_NACK;

                    /* Check if buffer content was modified: the address phase resets the locIndex */
                    if(locIndex != UART_1_EZI2C_GET_OFFSET(activeAddress))
                    {
                        locStatus |= UART_1_INTR_SLAVE_I2C_WRITE_STOP;
                    }

                    /* Complete read or write transaction */
                    locStatus &= locIntrSlave;
                    UART_1_EZI2C_UPDATE_LOC_STATUS(activeAddress, locStatus);
                    locStatus |= (uint32)  UART_1_curStatus;
                    locStatus &= (uint32) ~UART_1_EZI2C_STATUS_BUSY;
                    UART_1_curStatus = (uint8) locStatus;
                    ...
                    if(UART_1_SECONDARY_ADDRESS_ENABLE)
                    {
                        ...
                        UART_1_EZI2C_SET_INDEX(activeAddress, locIndex);
                    }
                ...
                */
                if(0u != (locIntrSlave & UART_1_INTR_SLAVE_I2C_ADDR_MATCH))
                {
                    if(UART_1_SECONDARY_ADDRESS_ENABLE)
                    {
                        /* Read address byte from RX FIFO */
                        locByte = UART_1_GET_I2C_7BIT_ADDRESS(UART_1_RX_FIFO_RD_REG);

                        UART_1_ClearRxInterruptSource(UART_1_INTR_RX_NOT_EMPTY);

                        /* Check received address against device addresses */
                        if(UART_1_addrBuf1 == locByte)
                        {
                            /* Set buffer exposed to primary slave address */
                            UART_1_dataBuffer = UART_1_dataBuffer1;
                            UART_1_bufSizeBuf = UART_1_bufSizeBuf1;
                            UART_1_protectBuf = UART_1_protectBuf1;

                            activeAddress = UART_1_EZI2C_ACTIVE_ADDRESS1;
                        }
                        else if(UART_1_addrBuf2 == locByte)
                        {
                            /* Set buffer exposed to secondary slave address */
                            UART_1_dataBuffer = UART_1_dataBuffer2;
                            UART_1_bufSizeBuf = UART_1_bufSizeBuf2;
                            UART_1_protectBuf = UART_1_protectBuf2;

                            activeAddress = UART_1_EZI2C_ACTIVE_ADDRESS2;
                        }
                        else
                        {
                            /* Address does not match */
                            ackResponse = UART_1_EZI2C_NACK_RECEIVED_ADDRESS;
                        }
                    }
                    

                #if(UART_1_SECONDARY_ADDRESS_ENABLE_CONST)
                    if(UART_1_EZI2C_NACK_RECEIVED_ADDRESS == ackResponse)
                    {
                        ...
                        UART_1_DISABLE_INTR_SLAVE(UART_1_INTR_SLAVE_I2C_STOP);
                    #endif /* (!UART_1_CY_SCBIP_V0) */
                ...
                #endif /* (UART_1_SECONDARY_ADDRESS_ENABLE_CONST) */
                    {
                        ...
                        UART_1_ENABLE_INTR_SLAVE(UART_1_INTR_SLAVE_I2C_STOP);
                    #endif /* (!UART_1_CY_SCBIP_V0) */
                ...
                UART_1_ClearSlaveInterruptSource(locIntrSlave);
            }
            ...
            */
            if(0u != (UART_1_GetInterruptCause() & UART_1_INTR_CAUSE_TX))
            {
                ...
                do
                {
                    ...
                    if(0u != locBufSize)
                    {
                        UART_1_TX_FIFO_WR_REG = (uint32) UART_1_dataBuffer[locIndex];
                        locIndex++;
                        locBufSize--;
                    }
                    else
                    {
                        UART_1_TX_FIFO_WR_REG = UART_1_EZI2C_OVFL_RETURN;
                    }
                }
                while(UART_1_EZI2C_FIFO_SIZE != UART_1_GET_TX_FIFO_ENTRIES);
    ...
    /*******************************************************************************
    * Function Name: UART_1_EZI2C_NO_STRETCH_ISR
    ********************************************************************************
    *
    * Summary:
        ...
        */
                                  ...
                                  UART_1_INTR_SLAVE_I2C_BUS_ERROR)))
        {
        ...
        #endif /* (UART_1_CY_SCBIP_V0) */
            {
                ...
                if(0u != (UART_1_EZI2C_FSM_WRITE_MASK & UART_1_fsmState))
                {
                    /* Write complete */
                    if(UART_1_indexBuf1 != UART_1_offsetBuf1)
                    {
        ...
        else
        {
            ...
            */
            if(0u != (locIntrCause & UART_1_INTR_CAUSE_RX))
            {
                ...
                case UART_1_EZI2C_FSM_BYTE_WRITE:
                    ...
                    if(UART_1_indexBuf1 < UART_1_protectBuf1)
                    {
                        /* Stores received byte into buffer */
                        UART_1_dataBuffer1[UART_1_indexBuf1] = (uint8) locByte;
                        UART_1_indexBuf1++;
                    }
                    else
                    {
                        /* Discard current byte and sets FSM state to discard following bytes */
                        UART_1_fsmState = UART_1_EZI2C_FSM_WAIT_STOP;
                    }
                ...
                case UART_1_EZI2C_FSM_OFFSET_LO8:
                    ...
                    if(locByte < (uint32) UART_1_bufSizeBuf1)
                    {
                        /* Store and sets received offset */
                        UART_1_offsetBuf1 = (uint16) locByte;
                        UART_1_indexBuf1  = (uint16) locByte;
                    ...
                    else
                    {
                        ...
                        UART_1_indexBuf1 = (uint16) UART_1_offsetBuf1;
                ...
                case UART_1_EZI2C_FSM_WAIT_STOP:
            ...
            if(0u != (locIntrSlave & UART_1_INTR_SLAVE_I2C_START))
            {
            #if(!UART_1_CY_SCBIP_V0)
                ...
                else
                {
                    ...
                    UART_1_ClearSlaveInterruptSource(UART_1_INTR_SLAVE_I2C_ADDR_MATCH);
                }
                ...
                UART_1_FAST_CLEAR_TX_FIFO;
                UART_1_TX_FIFO_WR_REG = (uint32) UART_1_dataBuffer1[UART_1_offsetBuf1];

                /* Store buffer index to be handled by INTR_SLAVE.I2C_STOP */
                locByte = (uint32) UART_1_indexBuf1;

                /* Update index: one byte is already in the TX FIFO */
                UART_1_indexBuf1 = (uint16) UART_1_offsetBuf1 + 1u;

                /* Enable INTR_TX.NOT_FULL to load TX FIFO */
                UART_1_SetTxInterruptMode(UART_1_INTR_TX_TRIGGER);

                /* Clear locIntrSlave after INTR.TX_TRIGGER is enabled */
                UART_1_ClearSlaveInterruptSource(locIntrSlave);

                locIntrCause |= UART_1_INTR_CAUSE_TX;
            }


            /* INTR_TX.TRIGGER */
            /* Transmits data to the master: loads data into the TX FIFO. The TX FIFO is loaded with data
            *  until used entries are less than UART_1_TX_LOAD_SIZE. If index reaches end of the
            *  buffer the 0xFF is sent to the end of transfer.
            */
            if(0u != (locIntrCause & UART_1_INTR_CAUSE_TX))
            {
                /* Put data into TX FIFO until there is room */
                do
                {
                    /* Check transmit buffer range */
                    if(UART_1_indexBuf1 < UART_1_bufSizeBuf1)
                    {
                        UART_1_TX_FIFO_WR_REG = (uint32) UART_1_dataBuffer1[UART_1_indexBuf1];
                        UART_1_indexBuf1++;
                    }
                    else
                    {
                ...
                while(UART_1_TX_LOAD_SIZE != UART_1_GET_TX_FIFO_ENTRIES);

                UART_1_ClearTxInterruptSource(UART_1_INTR_TX_TRIGGER);
            }
            ...
            if(0u != (locIntrSlave & UART_1_INTR_SLAVE_I2C_STOP))
            {
                ...
                if((locByte != UART_1_offsetBuf1) &&
                   (0u != (UART_1_EZI2C_FSM_WRITE_MASK & UART_1_fsmState)))
                {
                ...
                UART_1_curStatus |= (uint8) (locStatus & locIntrSlave);
            ...
            if(UART_1_EZI2C_EC_AM_ENABLE)
            {
                ...
                if(0u != (locIntrCause & UART_1_INTR_CAUSE_I2C_EC))
                {
                    UART_1_I2C_SLAVE_GENERATE_NACK; /* NACK in active mode */
                    UART_1_ClearI2CExtClkInterruptSource(UART_1_INTR_I2C_EC_WAKE_UP);
                }
            }
...
#endif /* (UART_1_EZI2C_SCL_STRETCH_DISABLE_CONST) */
